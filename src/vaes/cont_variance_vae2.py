...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        likelihood = Normal(mu2, std2) # log p(x, z) ~ log N(mu1, std2), S x B x eD
        ...
        return -torch.mean(elbos), torch.var(elbos, axis=0)/128
...
def test(epoch):
    ...
    with torch.no_grad():
        for i, (data, _) in enumerate(test_loader):
            ...
            test_loss += loss.item()
            test_elbo += elbo
            # recon_batch = model.sample_img(data)
...
if __name__ == "__main__":
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            ...
            sample, var = model.decode(sample)
